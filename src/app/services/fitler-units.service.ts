...
export class FitlerUnitsService {
  ...
  filterUnits(unit: Location, open_hour: string, close_hour: string) {
    ...
    let open_hour_int = parseInt(open_hour, 10)
    let close_hour_int = parseInt(close_hour, 10)
    ...
    for (let i = 0; i < unit.schedules.length; i++) {
      ...
      if (todays_weekday === schedules_weekday) {
        if (schedules_hour !== 'Fechada') {
          ...
          if (unit_open_hour_int <= open_hour_int && unit_close_hour_int >= close_hour_int) {
            return true
          }
        }
      }
  ...
  filter(results: Location[], showClosed: boolean, hour: string | null) {
